...
func Initialize(postgres *pgxpool.Pool, rdb *redis.Client) (err error) {
	...
	// GetInput, err = db.Prepare(`SELECT txid, vout, satoshis, outacc, lock, COALESCE(spend, '\x'::BYTEA), COALESCE(origin, '\x'::BYTEA)
	// 	FROM txos
	// 	WHERE spend=$1 AND outacc>=$2 AND satoshis=1
	// 	ORDER BY outacc ASC
	// 	LIMIT 1
	// `)
	// if err != nil {
	...
	// InsTxo, err = db.Prepare(`INSERT INTO txos(txid, vout, satoshis, outacc, lock, origin, height, idx, listing, bsv20)
	...
	// InsBareSpend, err = db.Prepare(`INSERT INTO txos(txid, vout, satoshis, outacc, spend, vin)
	// 	VALUES($1, $2, $3, $4, $5, $6)
	...
	// InsSpend, err = db.Prepare(`INSERT INTO txos(txid, vout, satoshis, outacc, lock, origin, height, idx, spend)
	...
	// SetTxn, err = db.Prepare(`INSERT INTO txns(txid, block_id, height, idx)
