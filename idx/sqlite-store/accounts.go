...
func (s *SQLiteStore) AcctsByOwners(ctx context.Context, owners []string) ([]string, error) {
	...
	rows, err := s.WRITEDB.QueryContext(ctx, query, toInterfaceSlice(owners)...)
	if err != nil {
		log.Panic(err)
		return nil, err
	}
...
func (s *SQLiteStore) AcctOwners(ctx context.Context, account string) ([]string, error) {
	rows, err := s.WRITEDB.QueryContext(ctx, `SELECT owner FROM owner_accounts WHERE account = ?`, account)
	if err != nil {
		log.Panic(err)
		return nil, err
	}
