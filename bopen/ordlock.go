...
func (i *OrdLockIndexer) Parse(idxCtx *lib.IndexContext, vout uint32) *lib.IndexData {
	...
	var origin *Origin
	if idxData, ok := txo.Data[ORIGIN_TAG]; ok {
	...
	if sCryptPrefixIndex := bytes.Index(*scr, OrdLockPrefix); sCryptPrefixIndex == -1 {
		...
		if idxData, ok := txo.Data[BSV21_TAG]; ok {
			...
			if origin.MimeType != "" {
				ordLock.Tags = append(ordLock.Tags, origin.MimeType)
			}
			ordLock.PricePer = float64(ordLock.Price)
			// for _, spend := range idxCtx.Spends {
			// 	if idxData, ok := spend.Data[ORIGIN_TAG]; ok && bytes.Equal(*origin.Outpoint, *idxData.Data.(*Origin).Outpoint) {
